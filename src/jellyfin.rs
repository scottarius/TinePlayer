...
pub fn discover(wait: std::time::Duration) -> Vec<Found> {
    let socket = match std::net::UdpSocket::bind("0.0.0.0:0") {
        Ok(socket) => socket,
        Err(e) => {
            log::error!("Couldn't open a socket to look for Jellyfin servers: {e}");
            return Vec::new();
        }
    };
    if let Err(e) = socket.set_broadcast(true) {
        log::error!("Couldn't broadcast to look for Jellyfin servers: {e}");
        return Vec::new();
    }
...
fn broadcast_addresses() -> Vec<std::net::Ipv4Addr> {
    ...
    let interfaces = match if_addrs::get_if_addrs() {
        ...
        Err(e) => {
            log::error!("Couldn't list this machine's networks: {e}");
            return addresses;
        }
    };
...
pub fn load() -> Option<Pairing> {
    ...
    match serde_json::from_str(&text) {
        ...
        Err(e) => {
            log::error!("Ignoring an unreadable Jellyfin pairing: {e}");
            None
        }
    }
}
