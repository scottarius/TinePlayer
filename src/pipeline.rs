...
impl AudioRouting {
    ...
    fn point(&mut self, role: &str, hub: Option<&gst::Element>) {
        ...
        let Some(src) = hub.request_pad_simple("src_%u") else {
            crate::log!("Failed to get a tee pad for the {role} output");
            return;
        };
        if let Err(e) = src.link(&sink) {
            crate::log!("Failed to feed the {role} output: {e}");
        ...
        .map_err(|e| e.to_string())?;
    {
        ...
        src.connect_pad_added(move |_, pad| {
            let Some(sink) = decode
                ...
                crate::log!("Failed to get a decoder sink pad for {}", pad.name());
                return;
            };
            if let Err(e) = pad.link(&sink) {
                crate::log!("Failed to link source to decoder: {e}");
            }
        });
    }
...

    {
        ...
        src.connect_pad_added(move |_, pad| {
            let Some(sink) = decode
                ...
                crate::log!("Failed to get a decoder sink pad for the audio file");
                return;
            };
            if let Err(e) = pad.link(&sink) {
                crate::log!("Failed to link the audio file to its decoder: {e}");
            }
        });
    }
...
pub fn attach_external_subtitle(
    ...
    // link waits for one to arrive rather than being made now.
    {
        ...
        src.connect_pad_added(move |_, pad| {
            ...
            if let Err(e) = pad.link(&sink) {
                crate::log!("Failed to link the subtitle source: {e}");
            }
        });
    }
...
fn add_audio_hub(
    ...
    let Ok(tee) = gst::ElementFactory::make("tee").build() else {
        crate::log!("Missing GStreamer element \"tee\". Check the install.");
        return;
    };
    ...
    if let Err(e) = pipeline.add(&tee) {
        crate::log!("Failed to add the tee for {name}: {e}");
        return;
    }
    ...
    if tee.sync_state_with_parent().is_err() {
        crate::log!("Failed to start the tee for {name}");
        return;
    }
    ...
    if let Err(e) = pad.link(&sink) {
        crate::log!("Failed to connect decoded audio on {name}: {e}");
        return;
    }
...
fn attach_pacer(pipeline: &gst::Pipeline, tee: &gst::Element) {
    let (Ok(queue), Ok(sink)) = (
        ...
        crate::log!("Could not build the pacing branch for an external audio file");
        return;
    };
    ...
    if pipeline.add_many([&queue, &sink]).is_err() {
        crate::log!("Failed to add the pacing branch for an external audio file");
        return;
    }
    if gst::Element::link_many([tee, &queue, &sink]).is_err() {
        crate::log!("Failed to link the pacing branch for an external audio file");
        return;
    }
    for element in [&queue, &sink] {
        if element.sync_state_with_parent().is_err() {
            crate::log!("Failed to start the pacing branch for an external audio file");
            return;
        }
    }
    ...
    decode.connect_pad_added(move |_, pad| {
        ...
        if let Target::Audio = target {
            ...
            let Some(playing) = playing else {
                crate::log!("Decoded audio on {} names no track we know", pad.name());
                return;
            };
        ...
        if let Err(e) = pad.link(&sink_pad) {
            crate::log!("Failed to connect decoded stream {id}: {e}");
...
fn discard(pipeline: &gst::Pipeline, pad: &gst::Pad) {
    ...
    if let Err(e) = pad.link(&target) {
        crate::log!("Couldn't discard the stream either: {e}");
        return;
    }
    if sink.sync_state_with_parent().is_err() {
        crate::log!("The discarded stream's sink would not start");
    }
}
